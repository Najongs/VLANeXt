...
{
  echo "# Stage 1 Sweep v2 — diff_steps × exec_steps on v3/1000 @ retreat=2"
  echo ""
  echo "Started: $(date)"
  echo "Ckpt: $CKPT"
...
for cell in "${CELLS[@]}"; do
    ...
    echo ""
    echo "=========================================="
    echo "[sweep_v2] $cell_label  $(date +%H:%M:%S)"
    echo "=========================================="
    ...
    if [ $rc -ne 0 ]; then
        echo "[sweep_v2] EVAL FAILED rc=$rc cell=$cell_label  (see $LOGDIR/${cell_label}.log)"
    ...
    if [ ! -d "$MERGED" ]; then
        echo "[sweep_v2] WARN merged missing — try _shard0"
    ...
    if [ -z "$MERGED" ] || [ ! -d "$MERGED" ]; then
        echo "[sweep_v2] no result dir found"
...
echo ""
echo "==========================================="
echo "Sweep v2 complete. Full results:"
echo "==========================================="
cat $RESULTS_MD
