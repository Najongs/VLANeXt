"""
sim_eval_insertion_only.py

Evaluates a fine-tuned VLANeXt model on insertion task.
(Pre-align to trocar entry with offset → model does approach + insertion)

Reset:
  1. Pre-align needle to trocar (IK, not recorded)
  2. Move to approach offset position (behind entry + XY offset)
  3. Model takes over: approach + insert + maintain alignment

Success: needle tip reaches target insertion depth along trocar axis
         while staying within lateral tolerance.

Usage:
    python -m scripts.sim_eval_insertion_only \
...
from src.datasets.sim_act_insertion import action_min_sim_insertion as action_min_sim, action_max_sim_insertion as action_max_sim
...
def _save_trajectory_plot(eval_dir):
    ...
    fig.suptitle(f"Insertion Eval Trajectories (n={len(trajectories)})", fontsize=14)
...
class InsertionSimEnv:
    """MuJoCo env for insertion evaluation.

    Reset:
      1. Optionally randomize phantom position
      2. Pre-align needle to trocar entry (IK)
      3. Move to approach offset position (behind entry + XY offset)
    Success: needle tip reaches insertion depth with acceptable lateral error
    """
    ...
    def __init__(self, model_xml_path: str, randomize_phantom: bool = False):
        ...
        self.insert_hold_counter = 0
        self.last_phantom_info = None
        self.last_start_info = None
    ...
    def reset(self):
        """Reset to approach offset position.
        Step 1: Restore aligned state (at entry)
        Step 2: Move back approach_offset along trocar axis
        Step 3: Apply XY offset
        """
        if self.randomize_phantom:
            self._aligned_qpos = None
        self._ensure_aligned_state()

        # Step 1: Restore aligned state (at entry 0.1mm)
        mujoco.mj_resetData(self.model, self.data)
        self.data.qpos[:self.n_motors] = self._aligned_qpos
        self.data.qvel[:self.n_motors] = self._aligned_qvel
        mujoco.mj_forward(self.model, self.data)

        axis_dir = (self._p_depth - self._p_entry)
        axis_dir = axis_dir / (np.linalg.norm(axis_dir) + 1e-10)
        curr_tip = self.data.site_xpos[self.tip_id].copy()
        curr_back = self.data.site_xpos[self.back_id].copy()
        needle_len = np.linalg.norm(curr_tip - curr_back)
        ...
        back_tip = self._p_entry - (axis_dir * approach_offset_m)
        back_back = back_tip - (axis_dir * needle_len)

        start_tip = self._goal_tip.copy()
        start_back = self._goal_back.copy()
        move_dist = np.linalg.norm(back_tip - start_tip)
        move_duration = max(move_dist / 0.05, 0.1)
        move_start_time = self.data.time

        for step in range(5000):
            t = (self.data.time - move_start_time) / move_duration
            alpha = smooth_step(min(t, 1.0))
            interp_tip = (1 - alpha) * start_tip + alpha * back_tip
            interp_back = (1 - alpha) * start_back + alpha * back_back
            self._run_ik_step(interp_tip, interp_back)
            mujoco.mj_step(self.model, self.data)
            if t >= 1.0:
                if np.linalg.norm(self.data.site_xpos[self.tip_id] - back_tip) < 0.001:
                    for _ in range(100):
                        self._run_ik_step(back_tip, back_back)
                        mujoco.mj_step(self.model, self.data)
                    break
                if step > 4500:
                    break

        # Step 3: Apply XY offset perpendicular to trocar axis
        up_vec = np.array([0.0, 0.0, 1.0])
        lateral1 = np.cross(axis_dir, up_vec)
        lat1_norm = np.linalg.norm(lateral1)
        if lat1_norm > 1e-6:
            lateral1 = lateral1 / lat1_norm
        else:
            lateral1 = np.array([1.0, 0.0, 0.0])
        lateral2 = np.cross(axis_dir, lateral1)
        lateral2 = lateral2 / (np.linalg.norm(lateral2) + 1e-10)

        xy_off1 = np.random.uniform(-APPROACH_XY_OFFSET_MM, APPROACH_XY_OFFSET_MM) / 1000.0
        xy_off2 = np.random.uniform(-APPROACH_XY_OFFSET_MM, APPROACH_XY_OFFSET_MM) / 1000.0
        xy_offset_vec = lateral1 * xy_off1 + lateral2 * xy_off2
        offset_tip = back_tip + xy_offset_vec
        offset_back = back_back + xy_offset_vec

        start_tip = self.data.site_xpos[self.tip_id].copy()
        start_back = self.data.site_xpos[self.back_id].copy()
        move_dist = np.linalg.norm(offset_tip - start_tip)
        move_duration = max(move_dist / 0.05, 0.1)
        move_start_time = self.data.time
        ...
        for step in range(5000):
            t = (self.data.time - move_start_time) / move_duration
            alpha = smooth_step(min(t, 1.0))
            interp_tip = (1 - alpha) * start_tip + alpha * offset_tip
            interp_back = (1 - alpha) * start_back + alpha * offset_back

            self._run_ik_step(interp_tip, interp_back)
            mujoco.mj_step(self.model, self.data)

            if t >= 1.0:
                if np.linalg.norm(self.data.site_xpos[self.tip_id] - offset_tip) < 0.001:
                    # Settle
                    for _ in range(200):
                        self._run_ik_step(offset_tip, offset_back)
                        mujoco.mj_step(self.model, self.data)
                    break
                if step > 4500:
                    break
        ...
        xy_offset_mm = np.linalg.norm(xy_offset_vec) * 1000.0
        ...
        print(f"  Start: offset={APPROACH_OFFSET_MM:.1f}mm behind, xy={xy_offset_mm:.1f}mm, "
              f"actual_dist={actual_dist:.1f}mm from entry")
    ...
    def check_success(self):
        ...
        if needle_len > 1e-8:
            cos_angle = abs(np.dot(needle_dir / needle_len, axis_dir))
        ...
        if success_cond:
            self.insert_hold_counter += 1
        else:
            self.insert_hold_counter = 0

        return self.insert_hold_counter >= INSERT_SUCCESS_HOLD_STEPS
...
def run_eval(cfg):
    ...
    try:
        ...
        step_str = "unknown"
    eval_dir = ckpt_path.parent / f"insertion_eval_step{step_str}_exec{num_steps_execute}_diff{diff_steps}{shard_suffix}"
    ...
    env = InsertionSimEnv(model_xml, randomize_phantom=randomize_phantom)
    ...
    csv_header = ["episode", "success", "steps", "final_dist_mm",
                   "final_insertion_depth_mm", "final_lateral_mm", "final_angle_deg",
                   "max_insertion_depth_mm",
                   "approach_offset_mm", "xy_offset_mm", "initial_dist_mm"]
    ...
    for ep in all_episodes:
        ...
        for ctrl_step in range(max_steps):
            ...
            # Insertion: gripper always closed (1.0)
            proprio_parts = [ee_pose, [1.0]]
            if use_sensor:
        ...
        max_depth = max(m["insertion_depth_mm"] for m in metrics_history)
        msg = (f"Episode {ep}/{num_episodes} | {'SUCCESS' if success else 'FAIL'} | "
               f"Steps: {ctrl_step + 1} | SR: {sr:.1f}% ({total_successes}/{ep_done}) | "
               f"depth={final_m['insertion_depth_mm']:.1f}mm lateral={final_m['lateral_mm']:.1f}mm "
               f"angle={final_m['angle_deg']:.1f}deg max_depth={max_depth:.1f}mm")
        print(msg)
        log_file.write(msg + "\n")
        log_file.flush()
        ...
        row = [
            ...
            f"{final_m['insertion_depth_mm']:.2f}",
            f"{final_m['lateral_mm']:.2f}", f"{final_m['angle_deg']:.2f}",
            f"{max_depth:.2f}",
            f"{si['approach_offset_mm']:.2f}",
            f"{si['xy_offset_mm']:.2f}",
    ...
    summary = f"\n{'='*60}\nFinal Insertion Success Rate: {final_sr:.2f}% ({total_successes}/{len(all_episodes)})\n{'='*60}"
    print(summary)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate VLANeXt on insertion task")
    parser.add_argument("--config", type=str, default="config/sim_eval_insertion_config.yaml")
